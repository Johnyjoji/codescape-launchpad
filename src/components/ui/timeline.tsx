    ...
    >
      <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
        ...
        <motion.h3
          ...
          animate={{ color: isInView ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground) / 0.5)" }}
        >
          {item.title}
        </motion.h3>
      </div>
...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className="w-full bg-section-alt font-sans md:px-10"
      ref={containerRef}
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        <h2 className="text-3xl lg:text-4xl mb-4 font-heading font-bold text-foreground max-w-4xl">
          Growth milestones
        </h2>
        <p className="text-muted-foreground text-sm md:text-base max-w-sm">
          Our journey from a bold idea to a thriving engineering company.
        </p>
      </div>
